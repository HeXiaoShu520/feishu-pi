...
1. **机器人配置检查**：未配置 FEISHU_APP_ID/SECRET 时自动弹出二维码，扫码创建/绑定应用并预置权限，写入 .env；
2. **管理员 lark 登录检查**：FEISHU_ADMIN 已配置但管理员尚未登录时，终端直接弹出授权链接/二维码——管理员本人扫码后**立即绑定管理员身份**（无需再去飞书发 /login，也无需重启）；
3. **团队名单上电解析**（后台）：遍历机器人所在会话的成员，全员按统一规则解析（lark-cli 用户态搜索：姓名+部门）写入用户名单缓存——之后任何人发消息，资料直接命中缓存。
