...
- **冷启动识别**：全新部署时用户缓存为空、通讯录权限可能未批，按姓名解析可能失败。此时请管理员**私聊机器人发送 `/login lark`** 完成登录——登录绑定成功时服务会自动识别管理员并把资料写入用户缓存（日志提示），**重启一次服务**后管理员权限即常态生效；此后每次启动直接走缓存解析，不再依赖通讯录权限。
- 管理员只由 `FEISHU_ADMIN` 配置（自动属于 admin 组）；`FEISHU_GROUP_ADMIN` 已废弃。组成员配置见 `.env.example`：`FEISHU_GROUP`（主团队组）、`FEISHU_GROUP_1/2…`，成员支持 open_id、中英文名或组织架构部门名。
