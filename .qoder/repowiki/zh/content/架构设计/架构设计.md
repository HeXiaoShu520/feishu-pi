...
## 更新摘要
**变更内容**   
- 从角色基础控制模型升级为基于组的策略驱动模型
- 新增 PolicyJudge 智能体审核组件，支持多模型并行判断
- 重构权限控制系统，支持多组并集策略和热重载
- 增强 ScheduleService 定时任务服务，支持持久化和状态管理
- 新增 SkillUsageStats 技能使用统计系统，独立事件流存储
- 移除旧的 UserRole 系统和安全守卫组件
...
**重要更新**：系统已从传统的角色基础控制模型升级为基于组的策略驱动模型，通过 PermissionPolicy 实现灵活的权限管理，结合 PolicyJudge 提供智能体综合判断能力，以及完整的定时任务和技能使用统计功能。
...
- 权限与 Guard：统一策略文件驱动多组身份，工具调用前经策略判定、智能体综合判断、授权卡确认三层闸门。
...
Policy["PermissionPolicy<br/>多组策略解析/组合并"]
Guard["ToolGuard + Broker<br/>策略→智能体→授权卡"]
Judge["PolicyJudge<br/>多模型并行审核"]
Schedule["ScheduleService<br/>cron调度/结果推送"]
Stats["SkillUsageStore<br/>技能使用统计"]
end
Lark --> Bridge
Bridge --> CM
CM --> Runtime
...
- [src/runtime/conversation-manager.ts:33-95](file://src/runtime/conversation-manager.ts#L33-95)
...
- **权限策略（PermissionPolicy）**：解析 .agent/permissions.json，计算 common ∪ 所属组的生效范围，支持多组并集、mtime 热重载，提供 bash/read/write/scripts/skills 判定函数。
- **智能体审核（PolicyJudge）**：配置 OpenAI 兼容接口，多模型并行判断，安全交集策略（全部 allow 才放行），超时或异常时 fallback 到人工审核。
- **工具调用 Guard（ToolGuard + PermissionBroker）**：策略命中免审放行；未命中走智能体综合判断；仍不确定则发授权卡由管理员单次确认；支持转发到管理员私聊。
- **定时任务（ScheduleService）**：持久化 cron 任务，启动时恢复调度，触发时以创建者身份运行智能体并将结果卡片推回目标会话；失败记录 lastStatus。
- **技能使用统计（SkillUsageStore）**：独立事件流存储，JSONL 格式追加写，支持用户展示名解析和技能读取检测。
...
Guard->>Judge : 策略未命中时智能体审核
Judge-->>Guard : allow/ask (多模型并行)
...
### 权限控制系统（PermissionPolicy + ToolGuard + Broker + PolicyJudge）
职责
- **策略文件**：.agent/permissions.json 定义 admin 与各用户组的能力（scripts/bash/read/write/tools），mtime 热重载。
- **会话级策略编译**：common ∪ 所属组并集，空缺省保守处理，支持一人多组能力叠加。
- **智能体审核**：PolicyJudge 配置 OpenAI 兼容接口，多模型并行判断，安全交集策略。
...
- 敏感参数脱敏展示，命令长度限制。
- 多模型审核：全部 allow 才放行，任一 ask 即弹卡，超时或异常 fallback 到人工审核。
...
D -- 否 --> J
E -- 否 --> J
J -- 是 --> Judge["PolicyJudge 多模型并行审核"]
Judge -- all allow --> Allow4["放行"]
Judge -- any ask --> Ask["发授权卡(Broker)"]
J -- 否 --> Ask
Ask --> Admin{"管理员批准?"}
Admin -- 是 --> Allow5["放行一次"]
Admin -- 否/超时 --> Block["拒绝"]
...
- 任务状态管理：lastRunAt、lastStatus、lastError 字段跟踪执行情况。

**章节来源**
- [src/schedule/service.ts:116-149](file://src/schedule/service.ts#L116-L149)
- [src/schedule/service.ts:212-235](file://src/schedule/service.ts#L212-L235)

### 技能使用统计（SkillUsageStore）
职责
- 独立事件流存储，JSONL 格式追加写，与 Pi session 无关。
- 技能读取检测：识别 read 工具的技能文件访问，记录 user × skill × time 维度。
- 用户展示名解析：英文名 > 中文名 > Open ID 优先级。

关键点
- 长期留存：不参与 DataCleaner 清理，支持历史趋势分析。
- 异步写入：内存缓存 + 串行写队列，失败只告警不影响工具执行。
- 用户缓存：复用 LarkCli 用户缓存文件，mtime 变化时重新加载。
...
Main --> Stats["skill-usage-store.ts"]
Runtime --> Policy
Runtime --> Guard
Runtime --> Stats
Guard --> Judge
...
- 常见问题定位
  ...
  - **智能体审核失败**：检查 guardBaseUrl/models/apiKey 配置，查看超时和模型响应日志。
  - **定时任务执行失败**：检查 cron 表达式有效性、runTask 注入、任务状态记录。
  - **技能统计缺失**：确认 SkillUsageStore 初始化、users 文件路径、JSONL 写入权限。
- 日志与观测
  - 传输层：WS 连接状态、消息归一化/分发错误。
  - 权限层：策略文件解析失败告警、bash 命令逃逸检测。
  - 运行时：工具调用 Guard 异常、技能使用记录失败。
  - 定时任务：cron 表达式无效、执行失败记录 lastError。
  - **智能体审核**：模型调用超时、响应解析失败、多模型判断结果。
  - **技能统计**：事件写入失败、用户缓存加载异常。
...
- [src/guard/judge.ts:68-119](file://src/guard/judge.ts#L68-L119)
- [src/stats/skill-usage-store.ts:84-157](file://src/stats/skill-usage-store.ts#L84-L157)

## 结论
mini-claw 以 Pi 为运行时底座，围绕飞书渠道构建轻量、可控、可审计的 Agent 平台。通过分层架构与基于组的策略驱动的权限体系，实现了"工具零改造、技能零侵入"的安全边界；借助 CardKit 流式卡片与精细化会话管理，提供了良好的用户体验。新增的智能体审核、定时任务和技能使用统计功能，进一步增强了系统的智能化和管理能力。未来可按业务需要逐步接入长期记忆与飞书业务能力，保持最小必要复杂度。
