# 安全控制

<cite>
**本文引用的文件**
- [src/guard/tool-guard.ts](file://src/guard/tool-guard.ts)
- [src/guard/broker.ts](file://src/guard/broker.ts)
- [src/guard/card.ts](file://src/guard/card.ts)
- [src/guard/judge.ts](file://src/guard/judge.ts)
- [src/runtime/feishu-pi-runtime.ts](file://src/runtime/feishu-pi-runtime.ts)
- [src/permission/policy.ts](file://src/permission/policy.ts)
- [src/utils/path-glob.ts](file://src/utils/path-glob.ts)
- [src/config.ts](file://src/config.ts)
- [src/scripts/runner.ts](file://src/scripts/runner.ts)
- [src/utils/logger.ts](file://src/utils/logger.ts)
- [src/feishu/commands.ts](file://src/feishu/commands.ts)
- [.agent/permissions.json](file://.agent/permissions.json)
- [test/tool-guard.test.ts](file://test/tool-guard.test.ts)
- [test/policy.test.ts](file://test/policy.test.ts)
- [docs/architecture.md](file://docs/architecture.md)
</cite>

## 更新摘要
**变更内容**
- 新增 bash 命令执行前密钥过滤机制，在策略评估之前拦截引用密钥/凭据文件的命令
- 增强运行时层密钥保护，通过系统提示注入密钥安全规则防止模型侧凭据泄露
- 完善工具守卫的密钥匹配逻辑，支持多种密钥文件模式（.env、*.key、凭据类等）
- 更新安全架构文档，反映双轨密钥防护机制（模型侧 + 执行侧）

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与资源限制](#性能与资源限制)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录：配置与审计监控](#附录配置与审计监控)

## 简介
本文件面向管理员与开发者，系统化说明本项目的安全控制机制，覆盖脚本执行边界、权限验证、访问控制、工具守卫（策略评估与执行限制）、进程隔离与资源限制、错误处理策略、安全配置、审计日志与监控方案，以及常见安全风险与防护措施。目标是提供完整的安全实施参考。

**更新** 本次更新重点增强了密钥保护机制，包括 bash 命令执行前的密钥过滤和运行时层的模型侧密钥安全规则，形成双轨防护体系。

## 项目结构
安全相关能力分布在以下模块：
- 权限策略：统一策略文件解析与组策略编译（策略层）
- 工具守卫：策略命中判定、智能体综合判断、授权卡审批（执行层）
- **密钥过滤**：bash 命令执行前密钥模式匹配（新增）
- **密钥规则**：运行时系统提示注入密钥安全约束（新增）
- 脚本执行器：子进程执行脚本，带超时与输出限制（执行边界）
- 卡片构建：授权/结果/提示卡片，参数脱敏展示（交互面）
- 路径匹配：glob 规则匹配 read/write/skills 范围（范围判定）
- 运行时钩子：在工具调用前注入守卫检查（集成点）
- 配置加载：环境变量驱动审核与授权超时等（配置面）
- 日志与命令：审计日志与 /perm 查看策略（可观测性）

```mermaid
graph TB
subgraph "策略层"
POL["PermissionPolicy<br/>策略解析与组合并"]
PGLOB["path-glob<br/>路径匹配"]
PJSON[".agent/permissions.json<br/>策略文件"]
end
subgraph "执行层"
GUARD["ToolGuard<br/>策略+智能体+授权卡"]
KEYFILTER["密钥过滤<br/>bash命令前拦截"]
RUNTIME["FeishuPiRuntime<br/>beforeToolCall 钩子"]
BROKER["PermissionBroker<br/>授权中枢"]
CARD["card.ts<br/>授权/结果/提示卡"]
end
subgraph "密钥防护"
SECRETRULE["密钥安全规则<br/>系统提示注入"]
KEYPATTERNS["密钥文件模式<br/>.env/*.key/凭据类"]
end
subgraph "执行边界"
RUNNER["scripts/runner.ts<br/>子进程执行脚本"]
end
subgraph "配置与可观测"
CFG["config.ts<br/>环境变量配置"]
LOG["logger.ts<br/>审计日志"]
CMD["commands.ts<br>/perm 查看策略"]
end
PJSON --> POL
POL --> PGLOB
RUNTIME --> GUARD
GUARD --> KEYFILTER
KEYFILTER --> BROKER
BROKER --> CARD
RUNTIME --> RUNNER
CFG --> RUNTIME
CFG --> GUARD
SECRETRULE --> RUNTIME
KEYPATTERNS --> KEYFILTER
LOG --- RUNTIME
LOG --- GUARD
LOG --- BROKER
CMD --- POL
```

**图表来源**
- [src/guard/tool-guard.ts:133-151](file://src/guard/tool-guard.ts#L133-L151)
- [src/runtime/feishu-pi-runtime.ts:91-99](file://src/runtime/feishu-pi-runtime.ts#L91-L99)
- [src/guard/broker.ts:46-199](file://src/guard/broker.ts#L46-L199)
- [src/guard/card.ts:27-86](file://src/guard/card.ts#L27-L86)
- [src/scripts/runner.ts:62-79](file://src/scripts/runner.ts#L62-L79)
- [src/config.ts:24-50](file://src/config.ts#L24-L50)
- [src/utils/logger.ts:24-39](file://src/utils/logger.ts#L24-L39)
- [src/feishu/commands.ts:287-313](file://src/feishu/commands.ts#L287-313)

章节来源
- [docs/architecture.md:82-112](file://docs/architecture.md#L82-L112)
- [docs/architecture.md:197-205](file://docs/architecture.md#L197-L205)

## 核心组件
- 统一权限策略（PermissionPolicy）
  - 从 .agent/permissions.json 加载并热重载；支持 common、owner 与任意用户组；按成员标识（openId/中文名/英文名）入组；合并为 GroupPolicy 判定函数（scripts/bash/read/write/skills）。
  - 默认保守：无组时仅技能目录可读，无脚本与命令权限；owner 未配置字段时全量放行。
- **工具守卫（ToolGuard）**
  - **新增密钥过滤**：bash 命令执行前先进行密钥模式匹配，命中直接拦截且不发授权卡
  - 每次工具调用前拦截：先策略命中放行，再交由智能体综合判断（allow/ask），否则走授权卡；read 范围外交直接拦截。
- **授权中枢（PermissionBroker）**
  - 管理待授权请求，发送授权卡片，服务端强制校验 approvalId/token/chatId/operatorOpenId，支持转发管理员私聊、精简模式撤回卡片、超时/中断收尾。
- **密钥防护机制**
  - **模型侧防护**：运行时注入密钥安全规则到系统提示，要求模型不读取/输出敏感文件，确需返回时必须星号遮蔽
  - **执行侧防护**：bash 命令执行前按空白与 shell 标点拆分 token，匹配密钥文件模式（.env、*.key、凭据类等）
- 脚本执行器（scripts/runner.ts）
  - 以子进程执行 .agent/scripts/*.ts，设置超时与输出上限，失败返回 stderr 摘要。
- 路径匹配（path-glob.ts）
  - 将 glob 编译为正则，支持 cwd 基准与相对/绝对路径匹配，防穿越路径误判。
- 配置加载（config.ts）
  - 通过环境变量配置审核接口、模型列表、超时与授权等待时间等。
- 运行时钩子（runtime）
  - 在 beforeToolCall 中注入守卫检查，异常回退为拒绝。
- 日志与命令（logger/commands）
  - 结构化日志；/perm 指令仅管理员可查看当前策略生效范围。

**章节来源**
- [src/permission/policy.ts:68-158](file://src/permission/policy.ts#L68-L158)
- [src/guard/tool-guard.ts:39-89](file://src/guard/tool-guard.ts#L39-L89)
- [src/guard/broker.ts:46-199](file://src/guard/broker.ts#L46-L199)
- [src/runtime/feishu-pi-runtime.ts:91-99](file://src/runtime/feishu-pi-runtime.ts#L91-L99)
- [src/scripts/runner.ts:62-79](file://src/scripts/runner.ts#L62-L79)
- [src/utils/path-glob.ts:10-41](file://src/utils/path-glob.ts#L10-L41)
- [src/config.ts:24-50](file://src/config.ts#L24-L50)
- [src/utils/logger.ts:24-39](file://src/utils/logger.ts#L24-L39)
- [src/feishu/commands.ts:287-313](file://src/feishu/commands.ts#L287-313)

## 架构总览
安全控制采用"静态策略 + 动态守卫 + 授权审批"的三层体系，**新增密钥防护双轨机制**：
- **密钥防护层**：模型侧系统提示约束 + 执行侧代码过滤，双重保障凭据安全
- 策略层：基于 .agent/permissions.json 的组策略，决定会话内可见工具与读写范围。
- 守卫层：对每次工具调用进行策略命中判定；未命中则调用智能体综合判断；仍不确定则发起授权卡。**bash 命令先经密钥过滤**。
- 执行层：脚本与 bash 以子进程运行，受超时与输出限制；read/write/edit 由宿主进程权限控制。

```mermaid
sequenceDiagram
participant U as "用户/智能体"
participant R as "FeishuPiRuntime"
participant G as "ToolGuard"
participant KF as "密钥过滤"
participant P as "PermissionPolicy"
participant J as "智能体(可选)"
participant B as "PermissionBroker"
participant C as "Card(授权/结果)"
U->>R : 工具调用
R->>G : beforeToolCall(groupPolicy, tool, args)
alt bash 命令
G->>KF : matchSecretCommand(command)
KF-->>G : 命中密钥模式?
alt 命中
G-->>R : 直接拦截(不发授权卡)
else 未命中
G->>P : 计算组策略
P-->>G : GroupPolicy
G->>J : judge() 综合判断
alt allow
G-->>R : 放行
else ask/未启用
G->>B : requestApproval()
B->>C : 发送授权卡
C-->>B : 管理员点击(allow_once/deny)
B-->>G : 决策结果
G-->>R : 放行或拒绝
end
end
else 其他工具
G->>P : 计算组策略
P-->>G : GroupPolicy
G->>J : judge() 综合判断
alt allow
G-->>R : 放行
else ask/未启用
G->>B : requestApproval()
B->>C : 发送授权卡
C-->>B : 管理员点击(allow_once/deny)
B-->>G : 决策结果
G-->>R : 放行或拒绝
end
end
R-->>U : 执行结果
```

**图表来源**
- [src/guard/tool-guard.ts:39-89](file://src/guard/tool-guard.ts#L39-L89)
- [src/guard/tool-guard.ts:133-151](file://src/guard/tool-guard.ts#L133-L151)
- [src/runtime/feishu-pi-runtime.ts:249-282](file://src/runtime/feishu-pi-runtime.ts#L249-L282)
- [src/guard/broker.ts:74-129](file://src/guard/broker.ts#L74-L129)
- [src/guard/card.ts:27-86](file://src/guard/card.ts#L27-L86)

## 详细组件分析

### 权限策略（PermissionPolicy）
- 功能要点
  - 读取 .agent/permissions.json，支持 mtime 热重载；common 与 owner 保留语义；其他组名自定义。
  - 成员匹配支持 openId、中文名、英文名（从 usersFile 缓存解析）。
  - 合并多组策略为并集；空读范围回退到技能目录；skills 空回退为全部。
  - 提供 describe() 用于 /perm 展示。
- 复杂度与性能
  - 组合并使用 Set 去重，O(n)；路径匹配使用预编译正则，单次 O(m)。
- 错误处理
  - 策略文件缺失/解析失败按保守默认处理；usersFile 解析失败忽略。
- 优化建议
  - 大策略文件时减少频繁 reload；按需懒加载 usersFile。

```mermaid
flowchart TD
Start(["进入 forGroups(groups)"]) --> Load["加载策略文件(含mtime热重载)"]
Load --> Merge["合并 common ∪ 各组(并集)"]
Merge --> Defaults{"read/skills 是否为空?"}
Defaults --> |是| ApplyDefaults["应用保守缺省"]
Defaults --> |否| Keep["保持原值"]
ApplyDefaults --> Build["生成判定函数(scripts/bash/read/write/skills)"]
Keep --> Build
Build --> End(["返回 GroupPolicy"])
```

**图表来源**
- [src/permission/policy.ts:109-158](file://src/permission/policy.ts#L109-L158)
- [src/permission/policy.ts:178-210](file://src/permission/policy.ts#L178-L210)

**章节来源**
- [src/permission/policy.ts:68-158](file://src/permission/policy.ts#L68-L158)
- [src/permission/policy.ts:178-210](file://src/permission/policy.ts#L178-L210)
- [test/policy.test.ts:14-121](file://test/policy.test.ts#L14-L121)

### 工具守卫（ToolGuard）
- 功能要点
  - **新增密钥过滤**：bash 命令执行前先进行密钥模式匹配，命中直接拦截且不发授权卡，对所有身份（含管理员）生效
  - 策略命中即放行；未命中则调用智能体 judge；judge 返回 allow 放行，返回 ask 或禁用时走授权卡。
  - read 范围外交直接拦截（能力问题不问人）。
  - scripts 名单外或标记 risky 时走授权卡。
- 错误处理
  - 智能体未配置/超时/异常 → 授权卡；Guard 自身异常 → 拒绝。
- 测试覆盖
  - 覆盖 bash 命中放行、策略未命中且 judge 禁用→拦截、judge allow/ask 分支、write 范围内外、scripts 名单内外与 risky 标记。
  - **新增**：bash 密钥指令过滤测试，包括 .env 引用、拼接引用、密钥文件引用、正常命令不受影响等场景。

```mermaid
flowchart TD
S(["check(policy, tool, args)"]) --> BashCheck{"toolName === 'bash'?"}
BashCheck --> |是| SecretFilter["matchSecretCommand(command)"]
SecretFilter --> SecretHit{"命中密钥模式?"}
SecretHit --> |是| Block["直接拦截(不发授权卡)"]
SecretHit --> |否| PolicyCheck["策略命中判定"]
BashCheck --> |否| PolicyCheck
PolicyCheck --> Hit{"策略命中?"}
Hit --> |是| Allow["放行"]
Hit --> |否| Judge{"智能体judge可用?"}
Judge --> |否| Ask["走授权卡(若无chatId则拦截)"]
Judge --> |是| Res{"decision=allow?"}
Res --> |是| Allow
Res --> |否| Ask
```

**图表来源**
- [src/guard/tool-guard.ts:39-89](file://src/guard/tool-guard.ts#L39-L89)
- [src/guard/tool-guard.ts:133-151](file://src/guard/tool-guard.ts#L133-L151)
- [test/tool-guard.test.ts:95-130](file://test/tool-guard.test.ts#L95-L130)

**章节来源**
- [src/guard/tool-guard.ts:39-89](file://src/guard/tool-guard.ts#L39-L89)
- [src/guard/tool-guard.ts:133-151](file://src/guard/tool-guard.ts#L133-L151)
- [test/tool-guard.test.ts:95-130](file://test/tool-guard.test.ts#L95-L130)

### 密钥防护机制
- **模型侧防护（系统提示）**
  - 运行时注入密钥安全规则到系统提示，要求模型不主动读取/输出 .env、密钥/证书/私钥/凭据类敏感文件
  - 确需返回敏感值时必须加星号遮蔽（只保留前几位，其余用 * 代替），不允许明文输出
- **执行侧防护（代码过滤）**
  - bash 命令执行前按空白与 shell 标点拆分 token，逐个匹配密钥文件模式
  - 支持的密钥模式：.env、*.env、*.key、*.pem、*.p12、*.pfx、id_rsa*、id_ed25519*、id_ecdsa*、*credential*、*secret*、.git-credentials、.netrc
  - 命中直接拦截，记录警告日志，不发授权卡
- **防护范围**
  - 密钥过滤仅针对 bash 工具，read/write 等其余工具不做代码拦截，靠模型侧规则约束
  - 对所有身份（含管理员）生效，无法绕过

**章节来源**
- [src/runtime/feishu-pi-runtime.ts:91-99](file://src/runtime/feishu-pi-runtime.ts#L91-L99)
- [src/guard/tool-guard.ts:133-151](file://src/guard/tool-guard.ts#L133-L151)
- [test/tool-guard.test.ts:95-130](file://test/tool-guard.test.ts#L95-L130)

### 授权中枢（PermissionBroker）
- 功能要点
  - 发起授权：生成唯一 approvalId 与一次性 token，发送授权卡片，等待管理员点击；支持转发管理员私聊。
  - 回调校验：严格校验 approvalId/token/chatId/messageId/operatorOpenId；决策者必须是管理员。
  - 收尾逻辑：精简模式撤回卡片，详细模式更新结果卡；支持会话中断取消。
- 安全性
  - 服务端强制校验，跨会话转发无效；授权一次有效，不缓存。
- 错误处理
  - 发送卡片失败直接拒绝；转发失败记录日志并拒绝；超时/中断统一收尾。

```mermaid
sequenceDiagram
participant G as "ToolGuard"
participant B as "PermissionBroker"
participant UI as "飞书卡片"
participant A as "管理员"
G->>B : requestApproval({toolName,args,chatId})
B->>UI : 发送授权卡(含approvalId/token)
A->>UI : 点击允许/拒绝/转发
UI->>B : handleCallback(params)
B->>B : 校验token/消息来源/管理员身份
B-->>G : {allowed : true/false}
B->>UI : 更新结果卡或撤回
```

**图表来源**
- [src/guard/broker.ts:74-129](file://src/guard/broker.ts#L74-L129)
- [src/guard/broker.ts:131-199](file://src/guard/broker.ts#L131-L199)
- [src/guard/card.ts:27-86](file://src/guard/card.ts#L27-L86)

**章节来源**
- [src/guard/broker.ts:46-199](file://src/guard/broker.ts#L46-L199)
- [src/guard/card.ts:27-86](file://src/guard/card.ts#L27-L86)

### 脚本执行器（scripts/runner.ts）
- 功能要点
  - 扫描 .agent/scripts/*.ts，解析元数据（description/input），包装为工具。
  - 以子进程执行脚本，设置超时与输出上限；失败返回 stderr 摘要。
- 信任边界
  - 脚本以子进程全权限运行；能写 scripts 目录（或推 git）的人即拥有进程。
- 错误处理
  - 非零退出码视为失败；输出截断防止过大响应。

```mermaid
flowchart TD
L(["loadScriptTools(dir)"]) --> Scan["扫描*.ts(排除.test.ts)"]
Scan --> Parse["解析元数据(description/input)"]
Parse --> Wrap["包装为工具{name, description, run}"]
Wrap --> Exec["run(input): execFile(超时/缓冲限制)"]
Exec --> Out{"成功?"}
Out --> |是| Ret["返回stdout(截断)"]
Out --> |否| Err["返回stderr摘要(截断)"]
```

**图表来源**
- [src/scripts/runner.ts:31-79](file://src/scripts/runner.ts#L31-L79)

**章节来源**
- [src/scripts/runner.ts:31-79](file://src/scripts/runner.ts#L31-L79)

### 路径匹配（path-glob.ts）
- 功能要点
  - 将 glob 编译为正则，支持 cwd 基准与相对/绝对路径匹配；双星加斜杠匹配多层目录。
- 安全性
  - 有 cwd 时同时测试 rel 与 abs 形态，避免穿越路径被原始字符串误判。

**章节来源**
- [src/utils/path-glob.ts:10-41](file://src/utils/path-glob.ts#L10-L41)

### 运行时钩子（runtime）
- 功能要点
  - 在 beforeToolCall 中注入守卫检查；技能读取命中时记录使用事件；Guard 异常按拒绝处理。
  - **新增**：系统提示注入密钥安全规则，约束模型行为。
- 错误处理
  - Guard 异常捕获并记录日志，返回 block=true。

**章节来源**
- [src/runtime/feishu-pi-runtime.ts:249-282](file://src/runtime/feishu-pi-runtime.ts#L249-L282)
- [src/runtime/feishu-pi-runtime.ts:91-99](file://src/runtime/feishu-pi-runtime.ts#L91-L99)

### 配置加载（config.ts）
- 关键配置项
  - 审核接口与模型：guardBaseUrl、guardModels、guardApiKey、guardTimeoutMs
  - 授权等待超时：approvalTimeoutMs
  - 工作目录与会话目录：cwd、sessionDir
- 环境变量驱动，便于不同环境部署与安全隔离。

**章节来源**
- [src/config.ts:24-50](file://src/config.ts#L24-L50)

### 日志与命令（logger/commands）
- 日志
  - 所有日志带时间戳与级别；支持 info/warn/error/log/userInput/aiResponse。
  - **新增**：密钥过滤警告日志，记录命中的密钥模式和命令。
- 命令
  - /perm 仅管理员可查看当前策略生效范围（common 与各组 effective）。

**章节来源**
- [src/utils/logger.ts:24-39](file://src/utils/logger.ts#L24-L39)
- [src/feishu/commands.ts:287-313](file://src/feishu/commands.ts#L287-313)

## 依赖关系分析
- PermissionPolicy 依赖 path-glob 进行路径匹配，依赖 logger 记录加载状态。
- ToolGuard 依赖 PermissionPolicy 提供的 GroupPolicy 判定函数，依赖 PermissionBroker 完成授权流程。
- **新增**：ToolGuard 依赖 path-glob 进行密钥文件模式匹配。
- PermissionBroker 依赖 card.ts 构建授权/结果/提示卡片，依赖 logger 记录异常。
- scripts/runner.ts 独立于策略层，但受策略层 scripts 白名单控制注册与调用。
- runtime 作为入口，串联策略、守卫与执行器。

```mermaid
graph LR
POL["PermissionPolicy"] --> PGLOB["path-glob"]
POL --> LOG["logger"]
GUARD["ToolGuard"] --> POL
GUARD --> PGLOB
GUARD --> BROKER["PermissionBroker"]
BROKER --> CARD["card.ts"]
RUNTIME["FeishuPiRuntime"] --> GUARD
RUNTIME --> RUNNER["scripts/runner.ts"]
RUNTIME --> SECRETRULE["密钥安全规则"]
```

**图表来源**
- [src/permission/policy.ts:68-158](file://src/permission/policy.ts#L68-L158)
- [src/guard/tool-guard.ts:133-151](file://src/guard/tool-guard.ts#L133-L151)
- [src/guard/broker.ts:46-199](file://src/guard/broker.ts#L46-L199)
- [src/scripts/runner.ts:31-79](file://src/scripts/runner.ts#L31-L79)
- [src/runtime/feishu-pi-runtime.ts:249-282](file://src/runtime/feishu-pi-runtime.ts#L249-L282)
- [src/runtime/feishu-pi-runtime.ts:91-99](file://src/runtime/feishu-pi-runtime.ts#L91-L99)

**章节来源**
- [src/permission/policy.ts:68-158](file://src/permission/policy.ts#L68-L158)
- [src/guard/tool-guard.ts:133-151](file://src/guard/tool-guard.ts#L133-L151)
- [src/guard/broker.ts:46-199](file://src/guard/broker.ts#L46-L199)
- [src/scripts/runner.ts:31-79](file://src/scripts/runner.ts#L31-L79)
- [src/runtime/feishu-pi-runtime.ts:249-282](file://src/runtime/feishu-pi-runtime.ts#L249-L282)

## 性能与资源限制
- 策略热重载：基于文件 mtime，新会话生效；避免频繁 I/O 重复解析。
- 路径匹配：预编译正则，单次匹配高效；支持 cwd 基准减少误判。
- **密钥过滤**：bash 命令按空白与 shell 标点拆分 token，使用预编译的正则进行模式匹配，开销极小。
- 脚本执行：30 秒超时、最大缓冲 1MB、输出截断至 8000 字符，防止资源耗尽。
- 授权等待：可配置超时，避免长时间阻塞；支持会话中断快速回收。
- 并发与队列：同一会话串行处理，新消息到达可中止在途请求，降低竞争。

## 故障排查指南
- 策略文件解析失败
  - 现象：策略加载失败，按保守默认处理（user 仅技能目录可读、无脚本/命令）。
  - 排查：检查 .agent/permissions.json 格式与 members 字段；确认 usersFile 存在且可读。
  - 参考：策略加载与容错逻辑。
- **密钥过滤误拦截**
  - 现象：正常的 bash 命令被拦截，提示引用了密钥/凭据文件。
  - 排查：检查命令中是否包含 .env、*.key、凭据类等文件名；确认是否为真正的密钥文件引用。
  - 参考：密钥文件模式匹配实现。
- 授权卡无法点击或无效
  - 现象：回调被拒绝，提示 token 或来源不匹配。
  - 排查：确认 approvalId/token 一致；operatorOpenId 属于管理员；消息来源为原卡或已转发卡。
  - 参考：授权回调校验与卡片来源校验。
- 脚本执行失败
  - 现象：非零退出码，返回 stderr 摘要。
  - 排查：检查脚本语法与输入参数；关注超时与输出大小限制。
  - 参考：脚本执行与错误处理。
- 路径读写被拦截
  - 现象：read/write 不在策略范围内被拦截。
  - 排查：确认 glob 规则与 cwd 基准；注意 .. 穿越路径不参与匹配。
  - 参考：路径匹配实现。
- 智能体审核异常
  - 现象：guard 异常导致拒绝。
  - 排查：检查 guardBaseUrl、guardModels、guardApiKey 与超时配置；确认网络连通。
  - 参考：运行时钩子中的异常处理。

**章节来源**
- [src/permission/policy.ts:178-210](file://src/permission/policy.ts#L178-L210)
- [src/guard/tool-guard.ts:133-151](file://src/guard/tool-guard.ts#L133-L151)
- [src/guard/broker.ts:131-199](file://src/guard/broker.ts#L131-L199)
- [src/scripts/runner.ts:62-79](file://src/scripts/runner.ts#L62-L79)
- [src/utils/path-glob.ts:10-41](file://src/utils/path-glob.ts#L10-L41)
- [src/runtime/feishu-pi-runtime.ts:261-282](file://src/runtime/feishu-pi-runtime.ts#L261-L282)

## 结论
本项目通过"策略层 + 守卫层 + 授权审批"的分层安全控制，实现了细粒度的访问控制与风险兜底。**新增的密钥防护双轨机制**进一步增强了凭据安全：模型侧通过系统提示约束模型行为，执行侧通过代码过滤拦截恶意命令。策略文件集中管理权限，守卫层确保每次调用可审计、可解释、可干预；授权卡提供人工兜底，结合超时与中断回收保障系统稳定性。配合路径匹配、脚本执行限制与日志审计，形成完整的安全闭环。

## 附录：配置与审计监控

### 安全配置指南
- 环境变量
  - 审核接口：FEISHU_GUARD_BASE_URL、FEISHU_GUARD_MODELS、FEISHU_GUARD_API_KEY、FEISHU_GUARD_TIMEOUT_MS
  - 授权超时：FEISHU_APPROVAL_TIMEOUT_MS
  - 工作目录与会话目录：FEISHU_PI_CWD、data/sessions
- 策略文件
  - 编辑 .agent/permissions.json，定义 common、owner 与用户组；members 支持 openId/中文名/英文名。
  - 使用 glob 精确限定 read/write/skills；bash 使用精确或前缀匹配，避免 shell 逃逸。
- 管理员与可见性
  - FEISHU_ADMIN 自动属于 owner；/perm 仅管理员可查看策略生效范围。
- **密钥防护配置**
  - 无需额外配置，密钥过滤和系统提示规则默认启用
  - 可通过调整密钥文件模式来定制防护范围

**章节来源**
- [src/config.ts:24-50](file://src/config.ts#L24-50)
- [.agent/permissions.json:1-35](file://.agent/permissions.json#L1-L35)
- [src/feishu/commands.ts:287-313](file://src/feishu/commands.ts#L287-313)
- [src/guard/tool-guard.ts:133-151](file://src/guard/tool-guard.ts#L133-L151)

### 审计日志与监控方案
- 日志
  - 所有关键操作（策略加载、授权回调、脚本执行、Guard 异常）均记录日志，带时间戳与级别。
  - **新增**：密钥过滤警告日志，记录命中的密钥模式和命令，便于安全审计。
- 监控建议
  - 收集授权卡通过率/拒绝率、超时次数、脚本执行失败率、路径拦截统计。
  - **新增**：监控密钥过滤触发频率，识别潜在的凭据泄露尝试。
  - 对 Guard 异常与策略解析失败设置告警阈值。
  - 定期导出 /perm 策略快照，对比变更。

**章节来源**
- [src/utils/logger.ts:24-39](file://src/utils/logger.ts#L24-39)
- [src/guard/broker.ts:58-72](file://src/guard/broker.ts#L58-L72)
- [src/scripts/runner.ts:62-79](file://src/scripts/runner.ts#L62-L79)
- [src/runtime/feishu-pi-runtime.ts:261-282](file://src/runtime/feishu-pi-runtime.ts#L261-L282)
- [src/guard/tool-guard.ts:47-52](file://src/guard/tool-guard.ts#L47-L52)

### 常见安全风险与防护措施
- 风险：脚本写入或系统命令执行导致越权
  - 防护：策略层 scripts/bash 白名单；子进程超时与输出限制；服务以低权限账号运行。
- 风险：路径穿越与越界读写
  - 防护：路径匹配使用 cwd 基准与正则；.. 穿越不参与匹配；read 范围外交直接拦截。
- 风险：授权卡伪造或跨会话劫持
  - 防护：服务端校验 approvalId/token/chatId/operatorOpenId；授权一次有效，不缓存。
- 风险：提示词注入绕过策略
  - 防护：策略判断在代码层执行；策略文件解析失败按最保守处理。
- **风险：凭据泄露**
  - **防护**：**双轨密钥防护** - 模型侧系统提示约束 + 执行侧代码过滤；凭据通过环境变量提供，不写入代码或日志。
- **风险：bash 命令绕过密钥防护**
  - **防护**：按空白与 shell 标点拆分 token 进行模式匹配，支持多种密钥文件模式；对所有身份（含管理员）生效。

**章节来源**
- [docs/architecture.md:197-205](file://docs/architecture.md#L197-L205)
- [src/guard/broker.ts:131-199](file://src/guard/broker.ts#L131-L199)
- [src/utils/path-glob.ts:10-41](file://src/utils/path-glob.ts#L10-L41)
- [src/scripts/runner.ts:62-79](file://src/scripts/runner.ts#L62-L79)
- [src/guard/tool-guard.ts:133-151](file://src/guard/tool-guard.ts#L133-L151)
- [src/runtime/feishu-pi-runtime.ts:91-99](file://src/runtime/feishu-pi-runtime.ts#L91-L99)