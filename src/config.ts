...
export interface FeishuPiAppConfig {
  ...
  modelName: string;
  modelBaseUrl?: string;
  /** 思考档位：low/high/max 三档，不同模型自动适配各自等效等级（FEISHU_PI_THINKING_LEVEL，默认 high） */
...
/** 思考档位：对外只提供三档，各模型运行时自动适配各自等效等级（pi 按模型目录夹取）。 */
const THINKING_LEVELS = ["low", "high", "max"] as const;
export type ThinkingLevelConfig = (typeof THINKING_LEVELS)[number];

/** 思考档位归一：只认 low/high/max；旧超集档位就近折算（minimal→low、medium→high、xhigh→max），其余回退 high。 */
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): FeishuPiAppConfig {
  ...
  return {
    ...
    // 思考档位只对外提供 low/high/max 三档，各模型自动适配等效等级。pi 默认 off（显式发
    // thinking:disabled 关思考），DeepSeek 官方默认=开+high，故本工程默认 high
