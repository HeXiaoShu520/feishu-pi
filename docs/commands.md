...
本项目实现以下内置指令，在飞书对话中直接输入使用：

- `/model` - 查看并切换 AI 模型（仅管理员可切换，即时生效）
- `/perm` - 查看身份说明与工具档位分布（仅管理员）
- `/help` - 显示帮助信息
...
管理员发送 `/perm` 后，Bot 返回一张卡片，包含：

- 内置组说明（`default` 全员、`admin` 超管）
- 通用层与负责人/用户两组的策略：工具、命令、可读、可写、技能范围
- 每个组的成员列表
- 每个技能（✦）/ 工具（⚙）按 permission 组的分布统计

数据来自 `FeishuPiRuntime.listToolPermissions()`，实时读取。非管理员收到「仅管理员可查看」提示。
...
**功能：**
- 从配置的中转站（`FEISHU_PI_MODEL_BASE_URL`）拉取可用模型列表，支持候选 URL 自动尝试（参考 cc-switch）
- 使用 CardKit 2.0 interactive 卡片展示模型列表
- 每个模型对应一个按钮，点击即切换：回调经服务端管理员校验后，调用 `runtime.setModelName()` 热切换（新会话立即生效），并持久化到 `.env`
...
- [x] 管理员权限验证（已实现，服务端校验）
- [x] 在配置中保存选中的模型（写入 .env）
- [x] 显示当前使用的模型
- [x] 实际的模型切换逻辑（runtime.setModelName 热切换，新会话生效）
