...
describe("ensureEnvFile（.env 缺失自动拷贝）", () => {
  ...
  it("已有 .env → 不覆盖；无 .env.example → 不动作", async () => {
    const dir = await mkdtemp(join(tmpdir(), "boot-"));
    await writeFile(join(dir, ".env"), "MINI_PI_VAULT_KEY=x\n", "utf8");
    expect(ensureEnvFile(dir)).toBe(false);
    expect((await readFile(join(dir, ".env"), "utf8")).includes("MINI_PI_VAULT_KEY")).toBe(true);
...
describe("ensureVaultKeyInEnv（主密钥落 .env）", () => {
  it("process.env 已有合法密钥 → 原样使用，不动 .env", async () => {
    ...
    const env = { MINI_PI_VAULT_KEY: HEX64 };
    ensureVaultKeyInEnv(dir, env as NodeJS.ProcessEnv);
    expect(env.MINI_PI_VAULT_KEY).toBe(HEX64);
    expect(existsSync(join(dir, ".env"))).toBe(false);
  });

  it(".env 已有密钥但环境未加载 → 补进环境", async () => {
    const dir = await mkdtemp(join(tmpdir(), "bootkey-"));
    await writeFile(join(dir, ".env"), `MINI_PI_VAULT_KEY=${OTHER_HEX64}\n`, "utf8");
    const env = {} as NodeJS.ProcessEnv;
    ensureVaultKeyInEnv(dir, env);
    expect(env.MINI_PI_VAULT_KEY).toBe(OTHER_HEX64);
  });
  ...
  it("无任何密钥但存在凭证数据 → 生成新密钥写入 .env 并清空旧凭证", async () => {
    ...
    const env = {} as NodeJS.ProcessEnv;
    ensureVaultKeyInEnv(dir, env);
    expect(env.MINI_PI_VAULT_KEY).toMatch(/^[0-9a-f]{64}$/);
    expect(env.MINI_PI_VAULT_KEY).not.toBe(HEX64);
    expect((await readFile(join(dir, ".env"), "utf8")).includes("MINI_PI_VAULT_KEY=")).toBe(true);
  ...
  it("全新环境（无密钥无凭证）→ 生成密钥写 .env，无清理动作", async () => {
    ...
    ensureVaultKeyInEnv(dir, env);
    expect(env.MINI_PI_VAULT_KEY).toMatch(/^[0-9a-f]{64}$/);
    await expect(readFile(join(dir, ".env"), "utf8")).resolves.toContain(`MINI_PI_VAULT_KEY=${env.MINI_PI_VAULT_KEY}`);
    // 再次执行：幂等（不换钥）
    const again = { ...env } as NodeJS.ProcessEnv;
    ensureVaultKeyInEnv(dir, again);
    expect(again.MINI_PI_VAULT_KEY).toBe(env.MINI_PI_VAULT_KEY);
  });
